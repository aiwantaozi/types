...
type MonitorGraphHandlerFunc func(key string, obj *MonitorGraph) error
...
type MonitorGraphController interface {
	...
	AddHandler(name string, handler MonitorGraphHandlerFunc)
	AddClusterScopedHandler(name, clusterName string, handler MonitorGraphHandlerFunc)
...
type MonitorGraphInterface interface {
	...
	AddHandler(name string, sync MonitorGraphHandlerFunc)
	AddLifecycle(name string, lifecycle MonitorGraphLifecycle)
	AddClusterScopedHandler(name, clusterName string, sync MonitorGraphHandlerFunc)
	AddClusterScopedLifecycle(name, clusterName string, lifecycle MonitorGraphLifecycle)
}
...
func (c *monitorGraphController) AddHandler(name string, handler MonitorGraphHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		...
		return handler(key, obj.(*MonitorGraph))
	})
}

func (c *monitorGraphController) AddClusterScopedHandler(name, cluster string, handler MonitorGraphHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *monitorGraphClient) AddHandler(name string, sync MonitorGraphHandlerFunc) {
	s.Controller().AddHandler(name, sync)
}

func (s *monitorGraphClient) AddLifecycle(name string, lifecycle MonitorGraphLifecycle) {
	sync := NewMonitorGraphLifecycleAdapter(name, false, s, lifecycle)
	s.AddHandler(name, sync)
}

func (s *monitorGraphClient) AddClusterScopedHandler(name, clusterName string, sync MonitorGraphHandlerFunc) {
	s.Controller().AddClusterScopedHandler(name, clusterName, sync)
}

func (s *monitorGraphClient) AddClusterScopedLifecycle(name, clusterName string, lifecycle MonitorGraphLifecycle) {
