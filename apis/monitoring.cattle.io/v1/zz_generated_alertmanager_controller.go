...
type AlertmanagerHandlerFunc func(key string, obj *Alertmanager) error
...
type AlertmanagerController interface {
	...
	AddHandler(name string, handler AlertmanagerHandlerFunc)
	AddClusterScopedHandler(name, clusterName string, handler AlertmanagerHandlerFunc)
...
type AlertmanagerInterface interface {
	...
	AddHandler(name string, sync AlertmanagerHandlerFunc)
	AddLifecycle(name string, lifecycle AlertmanagerLifecycle)
	AddClusterScopedHandler(name, clusterName string, sync AlertmanagerHandlerFunc)
	AddClusterScopedLifecycle(name, clusterName string, lifecycle AlertmanagerLifecycle)
}
...
func (c *alertmanagerController) AddHandler(name string, handler AlertmanagerHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		...
		return handler(key, obj.(*Alertmanager))
	})
}

func (c *alertmanagerController) AddClusterScopedHandler(name, cluster string, handler AlertmanagerHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *alertmanagerClient) AddHandler(name string, sync AlertmanagerHandlerFunc) {
	s.Controller().AddHandler(name, sync)
}

func (s *alertmanagerClient) AddLifecycle(name string, lifecycle AlertmanagerLifecycle) {
	sync := NewAlertmanagerLifecycleAdapter(name, false, s, lifecycle)
	s.AddHandler(name, sync)
}

func (s *alertmanagerClient) AddClusterScopedHandler(name, clusterName string, sync AlertmanagerHandlerFunc) {
	s.Controller().AddClusterScopedHandler(name, clusterName, sync)
}

func (s *alertmanagerClient) AddClusterScopedLifecycle(name, clusterName string, lifecycle AlertmanagerLifecycle) {
