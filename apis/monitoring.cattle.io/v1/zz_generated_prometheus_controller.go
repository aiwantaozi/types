...
type PrometheusHandlerFunc func(key string, obj *Prometheus) error
...
type PrometheusController interface {
	...
	AddHandler(name string, handler PrometheusHandlerFunc)
	AddClusterScopedHandler(name, clusterName string, handler PrometheusHandlerFunc)
...
type PrometheusInterface interface {
	...
	AddHandler(name string, sync PrometheusHandlerFunc)
	AddLifecycle(name string, lifecycle PrometheusLifecycle)
	AddClusterScopedHandler(name, clusterName string, sync PrometheusHandlerFunc)
	AddClusterScopedLifecycle(name, clusterName string, lifecycle PrometheusLifecycle)
}
...
func (c *prometheusController) AddHandler(name string, handler PrometheusHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		...
		return handler(key, obj.(*Prometheus))
	})
}

func (c *prometheusController) AddClusterScopedHandler(name, cluster string, handler PrometheusHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *prometheusClient) AddHandler(name string, sync PrometheusHandlerFunc) {
	s.Controller().AddHandler(name, sync)
}

func (s *prometheusClient) AddLifecycle(name string, lifecycle PrometheusLifecycle) {
	sync := NewPrometheusLifecycleAdapter(name, false, s, lifecycle)
	s.AddHandler(name, sync)
}

func (s *prometheusClient) AddClusterScopedHandler(name, clusterName string, sync PrometheusHandlerFunc) {
	s.Controller().AddClusterScopedHandler(name, clusterName, sync)
}

func (s *prometheusClient) AddClusterScopedLifecycle(name, clusterName string, lifecycle PrometheusLifecycle) {
