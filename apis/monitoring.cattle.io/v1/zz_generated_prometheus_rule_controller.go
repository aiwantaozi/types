...
type PrometheusRuleHandlerFunc func(key string, obj *PrometheusRule) error
...
type PrometheusRuleController interface {
	...
	AddHandler(name string, handler PrometheusRuleHandlerFunc)
	AddClusterScopedHandler(name, clusterName string, handler PrometheusRuleHandlerFunc)
...
type PrometheusRuleInterface interface {
	...
	AddHandler(name string, sync PrometheusRuleHandlerFunc)
	AddLifecycle(name string, lifecycle PrometheusRuleLifecycle)
	AddClusterScopedHandler(name, clusterName string, sync PrometheusRuleHandlerFunc)
	AddClusterScopedLifecycle(name, clusterName string, lifecycle PrometheusRuleLifecycle)
}
...
func (c *prometheusRuleController) AddHandler(name string, handler PrometheusRuleHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		...
		return handler(key, obj.(*PrometheusRule))
	})
}

func (c *prometheusRuleController) AddClusterScopedHandler(name, cluster string, handler PrometheusRuleHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *prometheusRuleClient) AddHandler(name string, sync PrometheusRuleHandlerFunc) {
	s.Controller().AddHandler(name, sync)
}

func (s *prometheusRuleClient) AddLifecycle(name string, lifecycle PrometheusRuleLifecycle) {
	sync := NewPrometheusRuleLifecycleAdapter(name, false, s, lifecycle)
	s.AddHandler(name, sync)
}

func (s *prometheusRuleClient) AddClusterScopedHandler(name, clusterName string, sync PrometheusRuleHandlerFunc) {
	s.Controller().AddClusterScopedHandler(name, clusterName, sync)
}

func (s *prometheusRuleClient) AddClusterScopedLifecycle(name, clusterName string, lifecycle PrometheusRuleLifecycle) {
