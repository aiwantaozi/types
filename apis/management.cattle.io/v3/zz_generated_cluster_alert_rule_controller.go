...
type ClusterAlertRuleHandlerFunc func(key string, obj *ClusterAlertRule) error
...
type ClusterAlertRuleController interface {
	...
	AddHandler(name string, handler ClusterAlertRuleHandlerFunc)
	AddClusterScopedHandler(name, clusterName string, handler ClusterAlertRuleHandlerFunc)
...
type ClusterAlertRuleInterface interface {
	...
	AddHandler(name string, sync ClusterAlertRuleHandlerFunc)
	AddLifecycle(name string, lifecycle ClusterAlertRuleLifecycle)
	AddClusterScopedHandler(name, clusterName string, sync ClusterAlertRuleHandlerFunc)
	AddClusterScopedLifecycle(name, clusterName string, lifecycle ClusterAlertRuleLifecycle)
}
...
func (c *clusterAlertRuleController) AddHandler(name string, handler ClusterAlertRuleHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		...
		return handler(key, obj.(*ClusterAlertRule))
	})
}

func (c *clusterAlertRuleController) AddClusterScopedHandler(name, cluster string, handler ClusterAlertRuleHandlerFunc) {
	c.GenericController.AddHandler(name, func(key string) error {
		obj, exists, err := c.Informer().GetStore().GetByKey(key)
		if err != nil {
			return err
...
func (s *clusterAlertRuleClient) AddHandler(name string, sync ClusterAlertRuleHandlerFunc) {
	s.Controller().AddHandler(name, sync)
}

func (s *clusterAlertRuleClient) AddLifecycle(name string, lifecycle ClusterAlertRuleLifecycle) {
	sync := NewClusterAlertRuleLifecycleAdapter(name, false, s, lifecycle)
	s.AddHandler(name, sync)
}

func (s *clusterAlertRuleClient) AddClusterScopedHandler(name, clusterName string, sync ClusterAlertRuleHandlerFunc) {
	s.Controller().AddClusterScopedHandler(name, clusterName, sync)
}

func (s *clusterAlertRuleClient) AddClusterScopedLifecycle(name, clusterName string, lifecycle ClusterAlertRuleLifecycle) {
